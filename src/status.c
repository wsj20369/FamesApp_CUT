...
#define __TEST_STYLE    LABEL_STYLE_BORDER
...
void init_produce_screen(gui_widget * parent)
{
    ...
    bad = gui_create_widget(GUI_WIDGET_FORM, X2, Y1, WIDTH2, HEIGHT2,
    ...
    widget = gui_create_widget(GUI_WIDGET_LABEL, 6, 54, 120, 32, 
                               PROD_COLOR, PROD_BKCOLOR, font24, 0|__TEST_STYLE);
    if(!widget)
        goto some_error;
    ...
    widget = gui_create_widget(GUI_WIDGET_LABEL, (116+ADJUST_X), 15, 472, 110, 
                               PROD_COLOR, PROD_BKCOLOR, font_big1, LABEL_ALIGN_RIGHT|__TEST_STYLE);
    if(!widget)
        goto some_error;
    gui_widget_link(prod, widget);
    gui_label_init_private(widget, 32);
